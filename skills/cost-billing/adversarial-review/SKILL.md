...
6. **Surface divergences — NEVER resolve a billable-surface conflict by inference.** When an artifact proposes a billable unit / classification that another stage or a provided source doc lacks or contradicts (e.g. a CPO Q4 feature whose `unit_hint` has no matching finance `billable_units[]`), R's job is to SURFACE the divergence, not collapse it. Record BOTH positions + the candidate unit as a `source_grounded_conflict` finding. R MUST NOT, on its own authority: **(a)** delete the proposed billable unit, **(b)** set or flip `internal_only` (a human-owned bootstrap field — CPO Q5), or **(c)** conclude "infrastructure / substrate / not billable" by architectural reasoning. Resolution must ground in the provided source docs (finance `sources_of_truth`, CPO Q2 `doc_sources`); if the docs don't adjudicate, escalate to the human in Phase 5. **A high-confidence signal that an upstream stage lacks is more likely an upstream CAPTURE GAP than an error to delete — absence upstream is NOT proof of "not billable."** (This rule exists because a billable unit can be silently deleted when a reviewer reasons "it's just infrastructure / the substrate" — burying a billable surface the customer's own spec doc defines, simply because the finance stage never ingested that doc. The reviewer should instead surface "CPO says billable, finance has no unit — which source is right?")
...
|---|---|---|---|
| `post-discovery` | `/cost-billing-discovery` Phase 5 (initial inventory build, pre-role-stages) | doc-tree + code-graph + draft inventories | Hallucinated features, false call edges, hallucinated `file:line`, double-mapped handlers, refund-test violations, catalog misses missed, misclassified matrix-hybrid pricing shape (a unit with an existing variable-quantity computation left `pricing_shape: flat`). |
...
| `post-codemod` | `/cost-billing-instrument` Phase 3 (per `--service`) | The PR(s) emitted by the codemod for one service | Compilation breaks, wrong adapter chosen, idempotency-key sloppiness, error paths un-instrumented, PII / security footguns, brownfield/greenfield mismatch, flat-value emission for a `pricing_shape: variable` unit (a hardcoded/coerced `value` shipped where the customer's own code already computes a variable per-occurrence quantity). |
...
|---|---|---|
| `post-discovery` | Hallucinated features, false call edges, hallucinated `file:line`, double-mapped handlers, refund-test violations | For each doc-tree leaf, search source docs (and `customer-context/product-summary.md`) for the claim — catch invented features. Spot-check 10% of code-graph edges via AST counter-claims. For each inventory entry: `head -n<line> <file> \| tail -1` to verify `file:line` matches the named handler. Run refund-test against each terminal-event candidate. **Matrix/hybrid pricing shape (`cost-billing-shared/matrix-hybrid-pricing.md`):** for each `usage-only`/`sibling-pair` candidate, grep the target function + its direct callees against `discovery/assets/terminal-event-heuristics.yaml`'s `variable_quantity_patterns` (metering vocabulary, vendor metering calls, ≥3-subclass override fanout on a base-class stub). Any hit where `refund_unit.pricing_shape` is still `flat` (or absent) is a CRITICAL miss — the entry ships a flat placeholder quantity while the customer's own code already proves the real cost varies per occurrence. |
...
| `post-codemod` | Compilation breaks, wrong adapter chosen, idempotency-key sloppiness, error paths un-instrumented, security footguns introduced by codemod, **dead-path emits (meter silently stays at ZERO), pre-commit emits (phantom/over-bill on rollback)** | Run customer's test suite (read-only — do NOT modify). Per-file: confirm adapter matches `customer-context/repo-info.yaml` framework. Audit each idempotency-key derivation site for variable scoping. Verify error/except paths don't emit usage events. Check inserted span attributes against `customer-context/telemetry-stack.yaml`'s `existing_attributes_prefix` — no collisions. PII-guard regex check on every inserted attribute name + value template. **Dead-path / liveness (the #572 dead-emit class):** every emit must sit in a function PRODUCTION actually calls. Run `shared/scripts/reachability.audit_emit_reachability(entries, repo_root)` over the inventory entries — any FLAG (`orphan` / `test_only` / `admin_e2e_only` / `error`) is a dead emit or an incomplete gate (the meter stays at ZERO; nothing crashes) → CRITICAL. `unverified` is NOT a pass — the static gate cannot prove liveness (dynamic dispatch / entry points are unbounded), so each `unverified` emit still owes a RUNTIME TRACE (trigger the billable action once, confirm the emit fires). Confirm the discovery `reachability` verdict was actually cleared (re-anchored or de-scoped), not ridden along. **Post-commit ordering (phantom-bill guard):** every billing emit must fire AFTER the transaction recording the billable action has COMMITTED. Trace each emit's position relative to the tx boundary — an emit INSIDE an open transaction (within `with session.begin()` / `db.transaction()`, inside a `@transactional` method before its return, or before an explicit `session.commit()`/`await tx.commit()`) phantom-bills on rollback (the action is undone but the bill was sent) and over-bills if the commit later fails → CRITICAL. The emit belongs AFTER the commit point — ideally via an outbox (a row written in the SAME tx, dispatched after commit) so the emit is atomic with the action. The retry-double-count `entity_id` dedup does NOT cover this: a rollback produces NO retry, so the phantom bill is never reconciled away. **Variable-quantity emission (matrix/hybrid pricing shape — `cost-billing-shared/matrix-hybrid-pricing.md`):** for every entry whose inventory record carries `refund_unit.pricing_shape: variable`, read the rendered call site's `value=` argument. It MUST reference a real runtime expression bound to the customer's existing quantity computation (typically an accumulator the codemod added, e.g. `self._moolabs_credits_consumed`) — a hardcoded literal (`value=1`) or an unaugmented single-call argument (when the real unit-of-work makes MULTIPLE contributing calls, e.g. a loop) is a CRITICAL finding: it silently mis-bills every occurrence whose real cost differs from the placeholder, and no downstream Moolabs-side pricing (including matrix/dimension-keyed rate schedules) can correct a wrong quantity. Cross-check `task_planner.py`'s own `pricing_shape_mismatch` flag on the entry (in `tasks.yaml`) — if `true`, the codemod already surfaced this at plan time and should have refused or required human confirmation before emitting; treat an emitted insert with `pricing_shape_mismatch: true` as a process failure to also flag. |
