...
Interactively walks the CFO through pricing model, billable units (with classification — customer-facing vs vendor-COGS vs sibling-pair), fair-usage thresholds, compliance regimes (regimes only — the sensitive-data categories they imply are enumerated at the CPO stage, field paths at the engineer stage), regions, and the commercial tenancy fact (is the product multi-tenant + billed per-tenant — the tenancy SHAPE is the CPO stage; the tenant_id field/source and environments are the engineer stage). Adversarial-reviews the draft before signoff. Outputs `01-finance.signed.yaml`.
