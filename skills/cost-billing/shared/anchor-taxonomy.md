...
- **How the codemod emits it:** `client.usage.ingest_events([...])` per the customer-facing Moolabs SDK.
...
- `client.meter.events.*` / `client.cls.*` are referenced in some older docs — **STALE. They never shipped. The live customer-facing namespace is `client.usage.*` (capability "usage" → EventsApi), verified against `moolabs-py` source. See `sdk-surface-reference.md`.**
