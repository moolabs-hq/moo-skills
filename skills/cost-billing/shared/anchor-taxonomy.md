...
- **How the codemod emits it (v0.3.0-rc1):** via `emit_cost_event_safe(args)` → `client.cost.ingest_event(args)` (singular ergonomic surface on `_CostNamespace`, routed to ACUTE backend via CostEventsApi + SdkIngestApi). The SDK's in-process buffer + structured-log recovery rail provide the never-drop guarantee; OTel spans are no longer a customer-chosen transport.
...
- **How the codemod emits it (v0.3.0-rc1):** via `emit_usage_event_safe(args)` → `client.usage.ingest_event(args)` (singular ergonomic surface on `_UsageNamespace`).
...
- **Cost-only** — cost-bearing call with no usage event (subscription customers; non-AI infra hot paths). v0.3.0-rc1 uses `emit_cost_event_safe()` → `client.cost.ingest_event(args)`. Never-drop is guaranteed by the SDK's in-process buffer + structured-log recovery rail; there is no separate OTel-span transport choice.
...
- `client.meter.events.*` / `client.cls.*` are referenced in some older docs — **STALE. They never shipped. The live customer-facing namespaces are `client.usage.*`, `client.cost.*`, and `client.events.*` (v0.3.0-rc1, verified against `moolabs-py` and `moolabs-ts` source). See `sdk-surface-reference.md`.**
