...
All three issues below verified at `moolabs-py@v0.2.0-rc9` source (independent code-read, not just relying on a customer bug report). **`moolabs-ts` NOT independently verified — same auto-generated lineage suggests same pattern, but treat the table below as Python-specific until cross-checked.**

| Issue | Symptom | Verified source | Workaround |
|---|---|---|---|
| **`base_url` mis-use → DNS mangling** | Customer passes `base_url="https://meter.dev.moolabs.com"` (a service URL, e.g. their `.env`'s `METER_BASE_URL`). SDK strips scheme and treats `meter.dev.moolabs.com` as the apex, then derives `ingest.us.meter.dev.moolabs.com` (rogue `meter.` segment), DNS NXDOMAIN. | `_dx_urls.py:resolve_effective_base_url` accepts apex strings but does NOT validate that `base_url` isn't already a service subdomain (`meter.` / `api.` / `acute.` / `ingest.`). Then `_dx_urls.py:_region_fallback_url_locked` (line ~502) mechanically prepends `ingest.<region>.`. | The codemod's helper template NEVER passes `base_url` — it constructs `Moolabs(api_key=...)` only, letting the SDK default to `moolabs.com`. Customers needing a non-default apex pass the apex (e.g. `dev.moolabs.com`), NOT the service URL. Upstream SDK fix: reject `base_url` strings that start with known service prefixes. |
| **URL path doubling** | Final ingest URL ends `/api/v1/events/api/v1/events`. | `_dx_urls.py:478` resolver returns the ingest URL with `_METER_INGEST_PATH = "/api/v1/events"` appended (constant at line ~47). `events_api.py:324` then calls `call_api(resource_path='/api/v1/events', ...)`. Composing the two yields the doubled path. | Upstream SDK fix needed at moolabs-hq/moolabs-py. Not the codemod's surface. |
| **Dev DNS topology gap** | SDK's regional ingest fallback returns `ingest.{region_code}.{base_url}` (verified `_dx_urls.py:~502`); customer's dev DNS doesn't expose `ingest.us.dev.moolabs.com`. | Platform-infra deployment gap, not an SDK code bug. SDK has a `step4_last_resort_url()` returning `meter.{base}/api/v1/events` as final fallback — that hostname DOES exist in dev. | Production deploys don't hit this. **Buffer mode (`Moolabs(api_key, buffer=True)`) reportedly returns `{'buffered': True}` without attempting the live POST — this is per a customer dev-probe report (2026-05-25), NOT independently verified by reading the SDK's buffer path. If you depend on it, verify against `_dx_buffer.py` first.** |
