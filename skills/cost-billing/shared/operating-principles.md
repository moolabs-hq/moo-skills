...
| "Their feature is called `chat.completed`, so `event_type=chat.completed` — confirm done." | Customer might call it `generation.delivered` or `message.completed`. Picking the wrong string means the codemod's emitted events don't match the Moolabs metering backend's rule. | Ask Q5 (event type naming + per-feature value) explicitly. |
