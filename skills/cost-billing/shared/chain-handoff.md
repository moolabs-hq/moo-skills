...
| **4. Team Engineer** | `01-finance.yaml` + `02-cpo.yaml` + `03-team-product.yaml` | Repo path(s); languages + frameworks; build + test commands; branch strategy; primary tracer + secondary instrumentation; request-context propagation pattern; attribute prefixes to avoid; agent surface + active LLM + MCP inventory + MCP selection + MCP restrictions; **SDK key location + read pattern**; **PII/PHI field-path translation** (maps CPO's sensitive-data categories → concrete handler field paths/regex the codemod's PII guard consumes — the data-model knowledge only the engineer has) | `04-final.yaml` (the consolidated customer-context all downstream skills read) |
