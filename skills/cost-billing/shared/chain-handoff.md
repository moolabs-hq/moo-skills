...
|---|---|---|---|
| **1. Finance** | (none — first in chain) | Pricing model TYPE + sub-aspects; pricing source of truth; billable units (CFO words); fair-usage + overages + bundling; per-customer custom pricing; compliance **regimes** (SOC2/HIPAA/GDPR/FedRAMP) — the regulatory regimes ONLY; **region(s)**; the **commercial tenancy fact** (is multi-tenant + billed per-tenant). NOTE: the sensitive-data *category* enumeration is NOT here (CPO), the multi-tenancy *shape* is NOT here (CPO), and field paths / tenant_id field / environments are NOT here (engineer) — finance owns neither the product's data model nor the code's field names. | `01-finance.yaml` |
| **2. CPO** | `01-finance.yaml` | Company + product names (+ vertical split if multi-product); product documentation source(s) — folders, URLs, MCPs; top features customer-enumerated; internal-only callouts; **sensitive-data categories** (which data the product handles that's regulated-sensitive under finance's regimes — debtor contact / payment / prompt bodies / PHI — product-domain knowledge the CPO owns; the engineer translates to field paths); end-user term; **multi-tenancy shape** (single / shared-db / isolated-db / workspace-based — product & data-architecture knowledge, informed by finance's commercial fact; the engineer confirms the tenant_id field/source); billable-output term; unique customer concepts | `02-cpo.yaml` |
...
| **4. Team Engineer** | `01-finance.yaml` + `02-cpo.yaml` + `03-team-product.yaml` | Repo path(s); languages + frameworks; build + test commands; branch strategy; primary tracer + secondary instrumentation; request-context propagation pattern; attribute prefixes to avoid; agent surface + active LLM + MCP inventory + MCP selection + MCP restrictions; **SDK key location + read pattern**; **PII/PHI field-path translation** (maps CPO's sensitive-data categories → concrete handler field paths/regex the codemod's PII guard consumes); **tenant_id field + request source** (the concrete code-level access path + JWT/subdomain/header/body, bound to the envelope's `customer_id`); **environments + per-env SDK-key location** — the data-model + deploy knowledge only the engineer has | `04-final.yaml` (the consolidated customer-context all downstream skills read) |
