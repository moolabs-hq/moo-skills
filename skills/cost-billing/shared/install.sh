...
#
# Cloud billing (AWS CUR) setup:
#   Two ways to run it:
#     1. Automatically — engineering/all personas are asked interactively
#        whether to set up the AWS Cost & Usage Report (CUR) integration
#        right after their skill install finishes.
#     2. Directly — `./install.sh --setup-cur` runs the same wizard on its
#        own, independent of any persona or skill install.
#   Either way, answering yes installs the moo-cloud-bill CLI and runs its
#   `configure` wizard for you (AWS profile selection, `configure` + `init` +
#   `verify`, then an offer to schedule the daily `push`).
#
# Personas:
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      # Print the whole leading comment block (lines 2..the first non-comment
      # line). Auto-adapts as the header grows — no hardcoded line range to
      # fall out of sync and silently truncate --help output.
...
#
# Two entry points share the same wizard (run_cur_setup_wizard) and the same
# explanatory banner — nothing about what/why changes based on how you got here:
#   1. Automatic — maybe_setup_cur() fires at the end of a normal install for
#      the engineering/all personas only.
#   2. Standalone — `./install.sh --setup-cur` calls run_cur_setup_wizard()
#      directly, before Main runs, so no persona prompt or skill install happens.
list_aws_profiles() {
...
# The actual CUR setup wizard: explains exactly what it will do, asks for
# confirmation, installs the moo-cloud-bill CLI, walks AWS profile selection +
# SSO login, runs `configure` + `init` + `verify`, then offers to schedule the
# daily `push`. Shared by both entry points (see header comment above).
#
# standalone=1 (only set by `--setup-cur`) turns a "nothing happened" outcome
# (no TTY, cli_dir missing, user declines, pip missing/fails) into a non-zero
# return — --setup-cur has no other install work to fall back on, so silently
# exiting 0 having done nothing would be misleading for scripted/CI callers.
# standalone=0 (the persona-gated path) keeps returning 0 in those cases, since
# there the rest of the install already succeeded.
run_cur_setup_wizard() {
  local cli_dir="$1"
  local standalone="${2:-0}"

  if [[ ! -d "$cli_dir" ]]; then
    echo "ERROR: cloud-bill-cli not found at $cli_dir (not bundled in this layout)." >&2
    [[ "$standalone" -eq 1 ]] && return 1
    return 0
  fi

  # Defensive: never mutate during a dry-run.
  ...
  if [[ ! -t 0 ]]; then
    echo "AWS CUR setup: skipped (non-interactive). Later:  pip install \"$cli_dir\" && moo-cloud-bill configure" >&2
    [[ "$standalone" -eq 1 ]] && return 1
    return 0
  fi
  ...
  # Install into an isolated sandbox by DEFAULT — this script must never touch
  # system-wide Python packages without asking first. pipx (a dedicated venv
  # per app) is preferred when present; otherwise we create our own throwaway
  # venv, which needs nothing but python3 and is just as isolated. A system/
  # global pip install is a LAST RESORT, offered only if the sandbox attempt
  # fails, and only after you explicitly say yes.
  local cloud_bill_venv="" sandbox_ok=0
  local install_log; install_log="$(mktemp)"

  if command -v pipx >/dev/null 2>&1; then
    echo "  Installing the CLI into an isolated pipx environment (sandboxed — no system-wide changes)…"
    if pipx install "$cli_dir" 2>&1 | tee "$install_log"; then sandbox_ok=1; fi
  elif command -v python3 >/dev/null 2>&1; then
    cloud_bill_venv="$HOME/.moolabs/cloud-bill-venv"
    echo "  Installing the CLI into a dedicated virtualenv at $cloud_bill_venv (sandboxed — no system-wide changes)…"
    if python3 -m venv "$cloud_bill_venv" 2>&1 | tee "$install_log" \
       && "$cloud_bill_venv/bin/pip" install "$cli_dir" 2>&1 | tee -a "$install_log"; then
      sandbox_ok=1
    else
      cloud_bill_venv=""   # half-built venv; don't let mcb_cmd point at it below
    fi
  else
    echo "  ! Neither pipx nor python3 found — can't set up a sandbox." | tee "$install_log" >&2
  fi

  if [[ $sandbox_ok -ne 1 ]]; then
    echo "  ! Sandboxed install did not succeed (see output above)." >&2
    if [[ ! -t 0 ]]; then
      echo "  Not falling back to a system-wide install non-interactively. Install manually:" >&2
      echo "      python3 -m venv $HOME/.moolabs/cloud-bill-venv && $HOME/.moolabs/cloud-bill-venv/bin/pip install \"$cli_dir\"" >&2
      rm -f "$install_log"
      [[ "$standalone" -eq 1 ]] && return 1
      return 0
    fi
    echo ""
    echo "  As a LAST RESORT I can install directly into your active/system Python"
    echo "  instead of a sandbox. This can affect packages other tools on this"
    echo "  machine rely on, and is harder to undo than deleting a venv."
    printf "  Proceed with a system-wide install? [y/N]: "
    read -r sys_confirm
    case "$sys_confirm" in
      y|Y|yes|YES)
        local syspip=""
        if command -v pip  >/dev/null 2>&1; then syspip="pip install"
        elif command -v pip3 >/dev/null 2>&1; then syspip="pip3 install"
        fi
        if [[ -z "$syspip" ]]; then
          echo "  ! no system pip found — install manually:  pip install \"$cli_dir\" && moo-cloud-bill configure" >&2
          rm -f "$install_log"
          [[ "$standalone" -eq 1 ]] && return 1
          return 0
        fi
        echo "  Installing system-wide ($syspip)…"
        if ! $syspip "$cli_dir"; then
          echo "  ! System-wide install failed too — install manually:  $syspip \"$cli_dir\" && moo-cloud-bill configure" >&2
          rm -f "$install_log"
          [[ "$standalone" -eq 1 ]] && return 1
          return 0
        fi
        ;;
      *)
        echo "  Skipped — nothing was installed. Install manually later:"
        echo "      python3 -m venv $HOME/.moolabs/cloud-bill-venv && $HOME/.moolabs/cloud-bill-venv/bin/pip install \"$cli_dir\""
        rm -f "$install_log"
        [[ "$standalone" -eq 1 ]] && return 1
        return 0
        ;;
  ...
  local mcb_cmd
  if [[ -n "$cloud_bill_venv" && -x "$cloud_bill_venv/bin/moo-cloud-bill" ]]; then
    mcb_cmd=("$cloud_bill_venv/bin/moo-cloud-bill")
  elif command -v moo-cloud-bill >/dev/null 2>&1; then
...
# Gated entry point used automatically by the normal install flow: only fires
# for the engineering/all personas, once per multi-platform fan-out. For a
# standalone run independent of persona/skill install, see the `--setup-cur`
# (SETUP_CUR) check right before Main, further down.
maybe_setup_cur() {
  ...
  run_cur_setup_wizard "$cli_dir" 0
}
...
# Standalone entry point: `--setup-cur` runs ONLY the CUR wizard, independent of
# persona selection and skill install. Placed here (before Main) so it short-
# circuits before platform detection / persona prompt / skill copy ever run.
if [[ $SETUP_CUR -eq 1 ]]; then
  run_cur_setup_wizard "$SUITE_SRC_DIR/cloud-bill-cli" 1
  exit $?
fi
