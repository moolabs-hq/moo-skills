...
def _load_module():
    spec = importlib.util.spec_from_file_location("attribution_map_signoff_contract", MODULE_PATH)
...
class AttributionMapSignoffContractTests(unittest.TestCase):
    ...
    def test_verify_requires_the_complete_attribution_signoff_shape(self) -> None:
        mutations = {
            ...
            "extra-signer-field": lambda value: value["signed_by"].__setitem__("unexpected", True),
            "extra-review-field": lambda value: value["adversarial_review"].__setitem__("unexpected", True),
            "extra-artifact-field": lambda value: value["artifact"].__setitem__("unexpected", True),
            "blank-signer-name": lambda value: value["signed_by"].__setitem__("name", "  "),
        }
    ...
    def test_verify_manually_validates_required_fields_types_timestamps_and_counts(self) -> None:
        mutations = {
            "missing-generated-at": lambda value: value.pop("generated_at"),
            "bad-generated-at": lambda value: value.__setitem__("generated_at", "yesterday"),
            "non-rfc3339-generated-at": lambda value: value.__setitem__(
                "generated_at", "2026-07-13 12:00:00+00:00"
            ),
            "signed-name-type": lambda value: value["signed_by"].__setitem__("name", 42),
            "bad-signed-at": lambda value: value["signed_by"].__setitem__("signed_at", "later"),
            "missing-review-count": lambda value: value["adversarial_review"].pop("findings_total"),
            "boolean-review-count": lambda value: value["adversarial_review"].__setitem__("findings_total", False),
            "negative-review-count": lambda value: value["adversarial_review"].__setitem__("findings_total", -1),
            "bad-review-time": lambda value: value["adversarial_review"].__setitem__("ran_at", "noon"),
            "review-evidence-type": lambda value: value["adversarial_review"].__setitem__("review_evidence", []),
            "artifact-risks-type": lambda value: value["artifact"].__setitem__("accepted_risks", "none"),
        }
        for name, mutate in mutations.items():
            with self.subTest(name=name):
                signoff = self._build()
    ...
    def test_schema_requires_codegen_model_and_review_evidence(self) -> None:
        ...
        attribution_branch = schema.split("stage: { const: engineer-attribution-map }", 1)[1]
