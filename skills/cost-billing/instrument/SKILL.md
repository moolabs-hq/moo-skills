...
7. **Live path only — never wire an emit into dead code.** Honor discovery's `reachability` verdict (`shared/scripts/reachability`). If an entry is FLAGGED (`orphan` / `test_only` / `admin_e2e_only`), the target function is not reached in production — wiring an emit there silently bills NOTHING (#572). REFUSE to emit; surface as CRITICAL with the re-anchor/de-scope ask. An `unverified` entry is not a pass — note in the PR that the engineer owes the runtime trace.
8. **Post-commit placement — never emit inside an open transaction.** Honor discovery's `tx_position` verdict (`shared/scripts/tx_guard`). The billable emit must be inserted AFTER the transaction recording the action COMMITS — an emit `inside_tx` / `before_commit` phantom-bills on rollback (the action is undone, the bill was sent) and the `entity_id` retry-dedup does NOT cover it (a rollback yields no retry). When the natural insertion point sits inside a transaction, place the emit after the commit / block exit (or recommend an outbox); if that can't be done safely, REFUSE and surface as CRITICAL rather than emit pre-commit.
