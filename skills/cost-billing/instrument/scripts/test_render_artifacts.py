...
class RenderAndWrite(unittest.TestCase):
    ...
    def test_real_template_roundtrip_regenerates(self):
        """The REAL stub template output must carry the generated marker so a
        SECOND run regenerates it (not skips it as a customer file). This is
        the round-trip the hand-injected test missed."""
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp)
            jobs = ra.plan_render_jobs(_tasks(), _TEMPLATES_DIR, repo)
            first = ra.render_and_write(jobs, repo, _TEMPLATES_DIR)
            self.assertTrue(any(m["action"] == "wrote" for m in first))
            stub = repo / "app/services/moolabs_settings.py"
            self.assertIn("/cost-billing-instrument", stub.read_text())
            # Second run: the prior output is recognized as ours → regenerated.
            second = ra.render_and_write(jobs, repo, _TEMPLATES_DIR)
            self.assertTrue(any(m["action"] == "regenerated" for m in second),
                            f"second run did not regenerate: {second}")

    def test_append_safe_guard_downgrades_full_file_kind(self):
        """Defensive: a full-file kind (terraform) wrongly given mode=append is
        treated as new_file, NOT appended into the existing file (corruption
        guard)."""
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp)
            tasks = _tasks(deployment_stubs=[
                {"kind": "terraform", "emit_path": "infra/moolabs.tf",
                 "mode": "append"},  # misconfigured: terraform is full-file
            ])
            jobs = ra.plan_render_jobs(tasks, _TEMPLATES_DIR, repo)
            manifest = ra.render_and_write(jobs, repo, _TEMPLATES_DIR)
            tf = repo / "infra/moolabs.tf"
            self.assertTrue(tf.is_file())
            # Written as a whole file (new_file), not appended.
            self.assertTrue(any(m["action"] in ("wrote", "regenerated")
...
class StubTemplateMarkerContract(unittest.TestCase):
    ...
    @unittest.skipUnless(_HAS_JINJA, "jinja2 not installed")
    def test_all_stub_templates_render_the_generated_marker(self):
        import jinja2
        env = jinja2.Environment(
            loader=jinja2.FileSystemLoader(str(_TEMPLATES_DIR)))
        for tpl in ("python-moolabs-settings.py.j2",
                    "typescript-moolabs-settings.ts.j2",
                    "go-moolabs-settings.go.j2"):
            rendered = env.get_template(tpl).render(service_slug="svc")
            self.assertIn(ra._GENERATED_MARKER, rendered,
                          f"{tpl} render lacks the generated marker → re-run "
                          f"would refuse to regenerate")
