...
- **Billing-surface gate (necessary-but-not-sufficient — apply to EVERY candidate)** — the refund-test score alone NEVER classifies a candidate as billable. Before emitting any terminal-event candidate as a billable usage event (`pattern: usage-only` or `sibling-pair`), confirm the product/service it belongs to has a billable surface in the authoritative model: **(1)** the CPO did NOT mark that product `internal_only`, **AND (2)** finance Stage 1 declares ≥1 `billable_units[]` whose scope covers it. If the product is `internal_only` OR has zero applicable billable units, **SUPPRESS every terminal-event candidate for it** — record each in `suppressions[]` with `suppression_category: internal_only_product` (or `no_billable_unit`) + rationale, regardless of how strongly the verb pattern or response shape scored. Apply the gate with `scripts/billing_gate.py`. This closes the gap the Vendor-COGS rule cannot: that rule iterates `billable_units[]`, so a product with *zero* units (pure infrastructure / internal tooling) has nothing to suppress against, and verb patterns like `_ingested`/`_generated`/`_aggregated`/`_delivered` sail through as phantom billable events. The authoritative finance+CPO model OVERRIDES the heuristic — a high refund-test score on a product with no billable surface is still suppressed.
