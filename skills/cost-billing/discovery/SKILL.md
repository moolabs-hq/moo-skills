...
- `sdk-surface-reference.md` — what to emit (`client.meter.events.ingest_events` for usage; cost via `client.cost.ingest_events_batch` when the snapshot reports it, OTel spans as the recovery rail)
