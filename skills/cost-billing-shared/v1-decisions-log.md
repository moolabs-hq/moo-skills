...
| 3 | Acute SDK status | **Blocking dependency for cost-only emission only.** v1 emits cost via **dual transport** (REVISED 2026-05-25 after moo-arc dogfood): preferred = OTel span attributes (when a recording span exists); fallback = structured log line (when no recording span) ingested by acute via the customer's log pipeline. Both arrive at the same downstream table. Cost events are **never silently dropped** by missing transport. Subscription-customer cost-only paths still get `# TODO: blocked on acute SDK` annotations until `client.acute.events.ingest_events()` ships, at which point the fallback swaps to direct SDK emission (call sites unchanged). | Per `sdk-surface-reference.md` — `client.acute.*` namespace not exposed yet. Original v1 default (span-only) would silently drop ~90% of cost signal under head-sampling (real production default), 100% under non-traced background workers / dev / CI; the never-drop contract makes the attribution engine actually trustworthy. | When SDK team adds `client.acute.events.ingest_events()`. |
...
| 9 | Skill C customer-facing exposure | **REMOVED FROM SUITE.** Skill C (attribution-engine reconciliation harness — validates moo-acute's algorithm ladder against real customer cloud bills) is Moolabs-engineering-internal infrastructure. It has no business running in a customer environment and was removed from this customer-portable suite. Tracked separately by the Moolabs platform team. | Doc 3 Appendix B Q5 + the skill content itself (paths to moo-acute internals, attribution_engine.py refs, ACUTE-Tier nomenclature) made clear it didn't belong here. | When Moolabs decides where the engineering-internal version lives (separate repo or moolabs-internal skills namespace). |
...
| §6.4a #19m Fair-usage data placement | **Carried in `usage-events-inventory.yaml`** (CFO-facing metadata block). Survives Skill 3 drift. | First-class CFO data; downstream moo-meter config reads from here. |
