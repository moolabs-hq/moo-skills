...
| 3 | Acute SDK status | **Blocking dependency for cost-only emission only.** v1 emits cost via OTel spans + Moolabs custom attributes (read by Tier 2/4 ingest). Subscription-customer cost-only paths get `# TODO: blocked on acute SDK` annotations. | Per `sdk-surface-reference.md` — `client.acute.*` namespace not exposed yet, but cost API classes are generated. OTel-span path is honest and unblocks v1. | When SDK team adds `client.acute.events.ingest_events()`. |
...
| §6.4b #19h Build-system integration | **Codemod does NOT run customer build commands.** PR carries a "to run before merge: `pip install -U moolabs && pytest` (or equivalent)" note. | Reproducibility for review. |
